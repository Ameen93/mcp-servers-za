...
export class StitchClient {
  ...
  constructor(config: StitchClientConfig) {
    this.config = {
      ...
      scopes: ["client_paymentrequest", "client_refund"],
      ...config,
    };
  }
  ...
  async getToken(): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new StitchAuthError(`Token request failed (${response.status}): ${text}`);
    }
  ...
  async query<T = unknown>(
    ...
    if (!response.ok) {
      ...
      const text = await response.text();
      throw new StitchApiError(`GraphQL request failed (${response.status}): ${text}`);
    }
    ...
    if (json.errors?.length) {
      ...
      throw new StitchApiError(err.message, code, err.extensions?.reason);
    }
